...
const expectedMediatorConfig: MediatorConfig = {
  ...
  name: 'MPI mediator',
  description:
    'A mediator handling interactions between the OpenHIM Core service, Sante MPI, Hapi-FHIR, and Kafka',
...
describe('Mediator Registration', () => {
  ...
  describe('Test Resolving of Mediator Config', () => {
    it('Should parse and return the mediatorConfig', () => {
      const mediatorConfig: MediatorConfig = resolveMediatorConfig(
        path.resolve(__dirname, './mediatorConfig-test.json')
      );
