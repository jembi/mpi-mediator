...
const fetchAllLinkedPatientResources = async (
  ...
  try {
    ...
    const fhirRequests = patientRefs.map(
      fetchAllPatientResourcesFromFhirDatastore
    );
    ...
    logger.error(
      'Unable to fetch all linked patient resources (MDM expansion)',
      e
    );
    return {
      status: 500,
 ...
 */
export const mpiMdmEverythingMiddleware: RequestHandler = async (
  req,
  res,
  next
) => {
  ...
  const { status, body } = await fetchAllLinkedPatientResources(
